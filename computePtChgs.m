function [ptChgs] = computePtChgs(numPtClds, TravelPathArray, ColorChange, PtCldArray, silent)
ptChgs = {};
BagIdx = 0;
...
% create a hash table on the first point cloud and populate it with points
...
for i=1:size( srcCloudPoint.backupVertexList, 2 )
    ...
    p = ptElt(coord1, color1, duration1);
    p.setIdx(i);
    hashMapOnFirstCldPt(hval)= p;
end
    ...
    for k=1:size(tgtTrvlPath{1},2)
        ...
        for hidx=hashMapIdx:-1:1
            if foundProbeKey == false && hashMapArrays{hidx}.isKey(probeKey)
                ...
                myElt=ptElt(coord1, color1, duration1);
                myElt.setIdx(vidx);
...
if ~silent
    ...
    outputT= ['Success!  Computed paths for ', num2str(size(ptChgs,2)),' FLSs successfully.'];
    disp(outputT);
