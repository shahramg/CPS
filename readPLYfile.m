function [vertexList, faceList, minW, maxW, minH, maxH, minD, maxD] = readPLYfile(filename)
...
for rowCount=1:numFaces
    currLine = textscan(fileID,'%s',1,'Delimiter','\n');
    currRow = char(currLine{1});
    splittedRow = strsplit(currRow,' ');
    splittedRow = str2double(splittedRow);
    faceList{rowCount} = [splittedRow(1) splittedRow(2)*multiplier splittedRow(3)*multiplier splittedRow(4)*multiplier splittedRow(5)*multiplier];
end
